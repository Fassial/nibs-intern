###################################
# Modified on 17:15, Nov. 6th, 2020
# Modifier: fassial
...
# def preprocess func
preprocess <- function(dir.data = DIR.DATA) {
    # read qc data
    cells <- readRDS(file.path(DIR.DATA, "cells.seurat_short.rds"))
    ...
    saveRDS(object = cells, file = file.path(DIR.DATA, "cells_preprocessed.rds"))
    return
}

# execute preprocess
preprocess(dir.data = DIR.DATA)
